...
import pickle
from danshuchu_dataset import DataGenerator
import combo_loss
...
class dataProcess(object):
    ...
    def __init__(self, out_rows, out_cols, data_path="./NII_TO_IMG/train/NPY", label_path="./NII_TO_IMG/train/NPY",
                 test_path="/media/lao/C14D581BDA18EBFA1/xss/Unet/work2/MFDA-Net/NII_TO_IMG/test_1/NPY1", img_type="bmp"):
        """
...
class myUnet(object):
    ...
    def dataAug(self, batch_imgs_v, batch_mask_v, batch_imgs_a, batch_size, mode, t):
        ...
        for i in range(len(batch_imgs_v)):
            img_v, mask_v, img_a = data_aug.data_aug(batch_imgs_v[i],batch_mask_v[i],batch_imgs_a[i], 0)
            imgs_v.append(img_v)
            imgs_a.append(img_a)
            masks.append(mask_v)
    ...
    def get_unet(self):
        ...
        conv1_v = Conv2D(32, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='dv_conv1_1')(
            inputs_v)
        conv1_v.trainable = True
        conv1_v = Conv2D(32, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='dv_conv1_2')(
            conv1_v)
        conv1_v.trainable = True

        conv1_a = Conv2D(32, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='da_conv1_1')(
            inputs_a)
        conv1_a.trainable = True
        conv1_a = Conv2D(32, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='da_conv1_2')(
            conv1_a)
        conv1_a.trainable = True
        pool1_a = MaxPooling2D(pool_size=(2, 2), name='a_pool1')(conv1_a)

        v_att1 = my_attention.attention_up_and_concate(conv1_v, conv1_a, name='av1_', train=True,
                                                       data_format='channels_last')
        pool1 = MaxPooling2D(pool_size=(2, 2), name='pool1')(v_att1)

        # (256,256,32)
        conv2_v = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='dv_conv2_1')(
            pool1)
        conv2_v.trainable = True
        conv2_v = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='dv_conv2_2')(
            conv2_v)
        conv2_v.trainable = True

        conv2_a = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='da_conv2_1')(
            pool1_a)
        conv2_a.trainable = True
        conv2_a = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='da_conv2_2')(
            conv2_a)
        conv2_a.trainable = True

        v_att2 = my_attention.attention_up_and_concate(conv2_v, conv2_a, name='av_2', train=True,
                                                       data_format='channels_last')
        pool2 = MaxPooling2D(pool_size=(2, 2), name='pool2')(v_att2)

        # (128,128,64)
        conv3_v = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='dconv3_1')(
            pool2)
        conv3_v.trainable = True
        drop3 = Dropout(0.1, name='ddrop3')(conv3_v)
        conv3_v = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='dconv3_2')(
            drop3)
        conv3_v.trainable = True

        conv3_v = my_attention.da_attention(conv3_v, name='da_attention_', train=True)
        pool3 = MaxPooling2D(pool_size=(2, 2), name='pool3')(conv3_v)

        # (64,64,128)
        conv4_v = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='dconv4_1')(
            pool3)
        conv4_v.trainable = True
        drop4 = Dropout(0.2, name='ddrop4')(conv4_v)
        conv4_v = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='dconv4_2')(
            drop4)
        conv4_v.trainable = True
        ...
        # (32,32,256)
        conv5_v = Conv2D(512, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='dconv5_1')(
            pool4_v)
        conv5_v.trainable = True
        drop5_v = Dropout(0.2, name='ddrop5')(conv5_v)
        conv5_v = Conv2D(512, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='dconv5_2')(
            drop5_v)
        conv5_v.trainable = True
        pool5_v = MaxPooling2D(pool_size=(2, 2), name='pool5')(conv5_v)

        ##(16,16,512)
        conv6_v = Conv2D(512, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='dconv6_1')(
            pool5_v)
        conv6_v.trainable = True
        drop6_v = Dropout(0.2, name='ddrop6')(conv6_v)
        conv6_v = Conv2D(512, 3, activation='relu', padding='same', kernel_initializer='he_normal', name='dconv6_2')(
            drop6_v)
        conv6_v.trainable = True
    ...
    def train(self):
        ...
        # plot_model(model, to_file='./result/da_add_av/cam_av/CAM_net_legend.png', show_shapes=True)   #被我注释

        # model.load_weights('./result/da_add_av/da_add_av.h5')
        # model = load_model('./result/DA_AV_attention_bs4/pre_aug_DA_A_V_att21.hdf5', custom_objects={'Combo_loss': combo_loss.Combo_loss})
        print("got unet")
        # model.save_weights('./result/da_add_av/by_name_da_add_av.h5')
        ...
        # dataset_root_path = "/media/lao/C14D581BDA18EBFA1/xss/Unet/dataset/NPY/"
        # img_v_floder = dataset_root_path + "test_img_v"
        # img_a_floder = dataset_root_path + "test_img_a"
        # mask_floder = dataset_root_path + "train_mask_v"
        #
        # img_v_list = os.listdir(img_v_floder)
        #
        # count = len(img_v_list)
        # print("length of test:", count)
        # test_imglist = img_v_list[:]
        ...
        #                               epochs=100,
        #                               callbacks=[
        #                                   keras.callbacks.ModelCheckpoint('./result/da_add_av/cam_av1/CAM_net.h5',
        #                                                                   monitor='val_loss',
        #                                                                   verbose=1,
        #                                                                   save_best_only=True,
        ...
        # plt.legend()
        # plt.savefig('./result/da_add_av/cam_av1/CAM_net')
        # with open('./result/da_add_av/cam_av1/CAM_net.txt', 'wb') as file_pi:
        #     pickle.dump(history.history, file_pi)
        #
        print('predict test data')
        model.load_weights('./result_DAM/position_1/DAM_1_net.h5')
        # for test_id in test_imglist:
        imgs_test_v_big, imgs_test_a_big = mydata.load_test_data()
            # input_data = [imgs_test_v_big, imgs_test_a_big]
            # print("input_data",input_data.shape)
        imgs_mask_test_big = model.predict([imgs_test_v_big, imgs_test_a_big], batch_size=1, verbose=1)
        print(imgs_mask_test_big.shape)
        np.save('./result_DAM/position_1/test_pred_1.npy', imgs_mask_test_big)  # 保存test的npy文件位置
    ...
    def save_img(self):  # 把测试的结果保存为bmp文件
        ...
        # imgs = np.load('./result/da_add_av/cam_av/CAM_net.npy')
        ...
        for i in range(pres.shape[0]):
            ...
            pre.save("./result/da_add_av/cam_av/CAM_pre1/" + str(i) + '.bmp')  # test结果另存一个文件夹
            # mask.save("./IMAGEV/test/MASK/" + str(i) + '.bmp')
            # img.save("./result/da_add_av/cam_av/CAM_pre/" + str(i) + '.bmp')
    ...
    def countdice1(self):
        ...
        for i in range(len(MASKNames)):
            ...
            # img22 = cv2.morphologyEx(img22, cv2.MORPH_CLOSE, kernel)
            # img22 = img22.reshape((img22.shape[0], img22.shape[1], 1))
