...
class myUnet(object):
    ...
    def train(self):
        if os.path.exists('./result_new/da_add_av/cam_av') == False:
            os.makedirs('./result_new/da_add_av/cam_av')
        model = self.get_unet()
        ...
        history = model.fit_generator(trainGene,
                                      ...
                                      epochs=100,
                                      callbacks=[
                                          keras.callbacks.ModelCheckpoint('./result_new/da_add_av/cam_av/CAM_net.h5',
                                                                          monitor='val_loss',
                                                                          verbose=1,
                                                                          save_best_only=True,
        ...
        plt.legend()
        plt.savefig('./result_new/da_add_av/cam_av/CAM_net')
        with open('./result_new/da_add_av/cam_av/CAM_net.txt', 'wb') as file_pi:
        ...
        # np.save('./result_new/da_add_av/cam_av/CAM_net.npy', imgs_mask_test_big)  # 保存test的npy文件位置
    ...
    def countdice1(self):
        ...
        PreNames = np.load('./result_new/fuse_way/merge_input/merge_input_big.npy')  # test 预测结果的位置
        ...
        Sen.to_csv('./result_new/fuse_way/merge_input/merge_input_big_sensitivity.csv')
        Spe.to_csv('./result_new/fuse_way/merge_input/merge_input_big_specificity.csv')
        os_rate = pd.DataFrame(OR)
        us_rate = pd.DataFrame(UR)
        os_rate.to_csv('./result_new/fuse_way/merge_input/merge_input_big_overrate.csv')
        us_rate.to_csv('./result_new/fuse_way/merge_input/merge_input_big_underrate.csv')
