...
def non_local_block(a_ip, v_ip, intermediate_dim=None, compression=2,
    ...
    if mode == 'gaussian':  # Gaussian instantiation
        x1 = Reshape((-1, channels))(a_ip)  # xi
        x2 = Reshape((-1, channels))(v_ip)  # xj
        f = dot([x1, x2], axes=2)
        f = Activation('softmax')(f)

    elif mode == 'dot':  # Dot instantiation
        # theta path
        theta = _convND(a_ip, rank, intermediate_dim)
        theta = Reshape((-1, intermediate_dim))(theta)

        # phi path
        phi = _convND(v_ip, rank, intermediate_dim)
    ...
    # g path
    g = _convND(v_ip, rank, intermediate_dim)
...
def a_nonlocal(a_layer, v_layer, data_format='channels_first'):
    ...
    a_layer = non_local_block(a_layer,intermediate_dim=None, compression=2, mode='embedded', add_residual=True)
