...
class myUnet(object):
    ...
    def get_unet(self):
        ...
        # # position 2
        # conv3_1 = my_attention.da_attention(conv3_1, name='da_attention_', train=True)
        ...
        # # position 3
        conv1_1 = my_attention.da_attention(conv1_1, name='da_attention_', train=True)
    ...
    def train(self):
        if os.path.exists('./result_DAM/position_3/') == False:
            os.makedirs('./result_DAM/position_3/')
        model = self.get_unet()
        ...
        # model.load_weights('./result_new/da_add_av/da_add_av.h5')
        # model.load_weights('./result_new/da_add_av/cam_av/CAM_net.h5')
        ...
        history = model.fit_generator(trainGene,
                                      ...
                                      epochs=100,
                                      callbacks=[
                                          keras.callbacks.ModelCheckpoint('./result_DAM/position_3/DAM_3_net.h5',
                                                                          monitor='val_loss',
                                                                          verbose=1,
                                                                          save_best_only=True,
        ...
        plt.legend()
        plt.savefig('./result_DAM/position_3/DAM_3_Net')
        with open('./result_DAM/position_3/DAM_3_net.txt', 'wb') as file_pi:
        ...
        # img = np.concatenate((imgs_test_v_big, imgs_test_a_big), axis=3)

        # multi_model.load_weights('./result/da_add_av/da_add_av.h5')
        model.load_weights('./result_DAM/position_3/DAM_3_net.h5')
        imgs_mask_test_big = model.predict([imgs_test_v_big, imgs_test_a_big], batch_size=1, verbose=1)
        np.save('./result_DAM/position_3/DAM_3_net.npy', imgs_mask_test_big)  # 保存test的npy文件位置
        # imgs_mask_test_small = model.predict([imgs_test_v_small, img_test_a_small], batch_size=1, verbose=1)
        # np.save('./result/fuse_way/merge_input/merge_input_small.npy', imgs_mask_test_small)
    ...
    def countdice1(self):
        ...
        # PreNames = np.load('./result_new/merge_input_big.npy')  # test 预测结果的位置
        ...
        # data1.to_csv('./result_new/fuse_way/merge_input/merge_input_big.csv')  # 把Dice数据存储到CSV中
        data1.to_csv('./result_new/merge_input_big.csv')  # 把Dice数据存储到CSV中

        sen, spe, OR, UR = sen_spe.Sen_spe(MASKNames, PreNames)
        Sen = pd.DataFrame(sen)
        Spe = pd.DataFrame(spe)
        Sen.to_csv('./result_new/merge_input_big_sensitivity.csv')
        Spe.to_csv('./result_new/merge_input_big_specificity.csv')
        os_rate = pd.DataFrame(OR)
        us_rate = pd.DataFrame(UR)
        os_rate.to_csv('./result_new/merge_input_big_overrate.csv')
        us_rate.to_csv('./result_new/merge_input_big_underrate.csv')
